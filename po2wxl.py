...
force = False
try:
    opts, args = getopt.getopt(sys.argv[1:], "hVfl:L:C:p:",
                               ["help", "version", "force", "langid=", "LCID=", "codepage=", "percentlimit="])
...
if po.percent_translated() <= translationPercentLimit:
    print("Skipping " + sourcefile + ": translated at " + str(po.percent_translated()) + "%, below the " + str(
          translationPercentLimit) + "% limit\n")
    sys.exit(0)
